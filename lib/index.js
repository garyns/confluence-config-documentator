...
updatePage = function(config, pageatts, content, callback) {
    ...
    if (config.macro && config.macro === "html") {
       d = updateHtmlMacro(pageatts.file, pageatts.body, content);
    } else if (config.macro && config.macro === "panel") {
...
updateHtmlMacro = function(file, body, content) {
    ...
    var matchesMacro = body.match(pattern);
    ...
    if (matchesMacro) {
        ...
        if (!codeBlockFound) {
            contentChanged = true;
            var hash = md5(content);
            body += createHtmlMacro(file + " (" + now + ") {" + hash + "}", content);
        }
...
updatePanelMacro = function(file, body, content) {
    ...
    if (matchesMacro) {
        ...
        if (!blockFound) {
            contentChanged = true;
            var hash = md5(content);
            body += createPanelMacro(file + " (" + now + ") {" + hash + "}", content);
        }
